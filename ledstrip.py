...
def colorWipeBlack(strip, wait_ms=10, iterations=208):
        """Wipe color across display a pixel at a time."""
        for j in range(iterations):
                for i in range(strip.numPixels()):
                        strip.setPixelColor(i, wheelBlack((i+j)));
                strip.show()
                time.sleep(wait_ms/1000.0)

def colorBlack():
        colorWipeBlack(strip)
